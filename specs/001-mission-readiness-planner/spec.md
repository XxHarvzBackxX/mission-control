...
4. **Given** a user has left one or more standard required fields blank (name, target body, mission type, or delta-v values), **When** the user attempts to save the mission, **Then** the system displays a `MissingRequiredField` warning identifying the missing information and does not save the mission.
5. **Given** a user has set mission control mode to "Crewed" but left the crew list empty, **When** the user attempts to save the mission, **Then** the readiness state is displayed as "Not Ready" with a `MissingCrew` warning, and the mission is not saved.
...
- **NFR-003**: Mission readiness calculations MUST complete in under 50ms end-to-end in the browser on a standard desktop — no perceptible delay following user input.
...
- **SC-002**: Readiness state and warnings are displayed within 50ms of saving mission parameters, with no perceptible delay.
